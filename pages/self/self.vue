	...
	<view class="user">
		<view class="top">
			<view class="group">
				<view class="userinfo">
					<view class="pic">
						<image :src="userInfo.avatar_file.url" v-if="hasLogin&&userInfo.avatar_file&&userInfo.avatar_file.url" mode="aspectFill">
						</image>
						<image v-else src="../../static/images/user-default.jpg" mode="aspectFill"></image>
					</view>
					<view class="text" v-if="hasLogin" @click="toUserInfo">
			...
			<view class="bg">
				<image v-if="hasLogin&&userInfo.avatar_file&&userInfo.avatar_file.url" :src="userInfo.avatar_file.url" mode="aspectFill">
				</image>
				<image v-else src="../../static/images/user-default.jpg" mode="aspectFill"></image>
			</view>
		</view>
		<view class="main">
			<view class="info">
				<view class="item"><text>{{totalObj.likeNum==undefined?0:totalObj.likeNum}}</text>获赞</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			if (this.hasLogin) this.getTotal()
		},
		methods: {
			async getTotal() {
				...
				let likeCount = await db.collection('quanzi_article').where('user_id==$cloudEnv_uid').groupBy('user_id').groupField(
					'sum(like_count) as likeCount').get()
				this.totalObj.likeNum = likeCount.result.data[0]?.likeCount
			},
			// 跳转到反馈
			feedback() {
